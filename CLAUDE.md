...
- **采集产能 Sheet 为空表** → 采集产能 Tab 显示空态，待用户填月份/计划·实际家数条数/人数
- **加工预审 col9 价格审核结果、预算结算 col7-11 去年值 全空** → 加工审核"价格审核通过率"、费用"去年对比"显示"—"，待用户填数
...
- **解析器验证**：`python parse_template.py 看板数据录入模板-调整.xlsx test.json` → 输出与磁盘模板缓存逐字段对比（settlements sum=902,077.2，21任务/26结算/3725加工预审；21/26 是磁盘模板真实值，HEAD data.json 的 19/25 是旧值，以模板为准）
...
### 预算与结算（23列，数据行 row3 起，max_row=28）
1=月份 2=产品模块 3=需求类型 4=需求数量 5=结算单价 **6=支出费用(公式=D*E)** 7=去年产品模块 8=去年需求类型 9=去年需求数量 10=去年结算单价 11=去年支出费用；16=结算一览(合并P2:W2，每月结算明细副表)
- **结算金额 = 需求数量×结算单价**（col6 公式无缓存，解析器自算）；去年结算金额同理 = 去年数量×去年单价
...
### 采集产能（7列，**当前为空表**待用户填）
1=月份 2=计划采集家数 3=计划采集条数 4=实际采集家数 5=实际采集条数 6=员工总人数 7=备注
...
3. **前端浏览器导入 TPL_COL 为 0-indexed**（=1基col−1，与 Python COL_MAP 一一对应）：
   ...
   - settlements: `{month:0, module:1, type:2, qty:3, price:4, cost:5, lastModule:6, lastType:7, lastQty:8, lastPrice:9, lastCost:10}`
   - preReview: `{reviewer:2, month:1, isNew:6, passed:7, priceAudit:8, reason:9}`
   - capacity: `{month:0, planHouse:1, planItem:2, actHouse:3, actItem:4, staff:5, note:6}`
