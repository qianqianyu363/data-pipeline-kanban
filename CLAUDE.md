...
| `parse_template.py` | Python 解析器 → data.json（**含对话台账合并（新增/补充模板空字段）+ 质量抽审公式求值 + 08-10 新版模板适配 + 08-17 各项需求跟踪列右移 + 08-18 加工预审 by_month 月度分解(new/old/price_audit/通过率)**） |
| `需求下发登记.json` | **对话登记台账**（Claude 对话写入，解析器合并进 data.json） |
| `index.html` | 看板前端（**7项优化 + UI指标去重修复 + dataEmbed 内嵌块自动同步 + 08-10 浏览器导入适配/产能需求量展示/质量抽审按月过滤 + 三模块可视化优化 + 08-17 采集完成时间/闭环状态列 + 08-18 任务清单改版(去时间列/项数条数分组) + 流程卡按月联动(结算累计执行/加工审核月度)**） |
...
### 用户模板改动（2026-08-17 更新 `_升级版.xlsx`）
- 各项需求跟踪 27→29列：新增「**采集完成时间**」(col22，日期序列号 46234→2026-07-31)、「**闭环状态**」(col29，值'是')；采集完成量/实际采集材料/完成率/无效数据/无效占比/发布数据 整体右移1位
- 加工预审 新增 col15「报价材料是否少于10条」（数据行全空，解析器不读，汇总区 c16 起不变）
- 采集产能 新增 c10/c11 辅助公式列（人均产能预测，解析器忽略，仍读 9 列）
- 数据更新：7月新增2任务（江苏/浙江比价）、8月新增3任务（山东/河北/湖北比价）；结算新增 7月4条 + 6月信息价补填 → **结算 sum 902,077.2 → 1,218,356.6**

### 解析器适配（parse_template.py）
1. COL_MAP/主循环：collected 22→23、completionRate 24→25、invalidData 25→26、invalidRate 26→27、published 27→28
2. 新增字段：`collectedTime`（col22，新 `fmt_collect_time` 日期序列号→'YYYY-MM-DD'）、`closed`（col29，'是'/'否'）
3. 其余 sheet（预算结算/加工预审/质量抽审/问题反馈/采集产能）列结构未变

### 前端适配（index.html）
- TPL_COL.tasks 0-indexed 右移同步（collected 21→22、completionRate 23→24、invalidData 24→25、invalidRate 25→26、publishedData 26→27）+ 新增 `collectedTime:21` / `closed:28`
- 新增 `fmtCollectTime`：序列号/MMDD/文本日期 → 'YYYY-MM-DD'/'MM月'（与 Python fmt_collect_time 一致）
- 任务清单：新增「采集完成」列（collectedTime）；「闭环状态」列优先取模板 closed（'是'→绿/否→灰），缺省回退完成率推断（旧数据/台账）
- 其余渲染字段名不变，月度趋势/各 Tab 聚合不受影响

### 验证
- 26任务/51结算/4492加工预审；结算 sum=1,218,356.6、去年支出 sum=491.3万
- 列右移抽查：四川比价 collected=5109/完成率88.7、7月江苏比价 collectedTime=2026-07-31、closed 1-7月=是/8月空
- 结算新旧对比：无列错位，差异全为模板新数据（7月结算+6月补填）
- `node --check` 通过、`_smoke_test.js` 5/5 通过；`fmtCollectTime` 单测通过（46234→2026-07-31、730→07月）
- 主模板已备份至 `.workbuddy/backup_20260817/` 后覆盖为升级版
...
### 前端可视化优化（2026-08-10 用户反馈，仅改 index.html）

1. **采集产能双象限**：cpChart1 拆成左右两图（cpChart1a 家数+人均家数 / cpChart1b 条数+人均条数），各自独立刻度消除家数·条数量级干扰；actHouse/actItem/staff 改为保留 null（修 `num()` 把缺失当 0 造成的假 0 柱/假人均折线），产能预测输入先截掉尾部 null
2. **费用执行**：
   - **根因修复**：settlements 含 1-12 月（未来月结算金额=0），原代码按 `monthNames` 最后判「截至12月」且 `monthsToPredict=0` → 预测费用虚线框与 predBox 实际失效。现改按 `lastActiveIdx`（最后一个结算金额>0 月份）→ 累计执行显示「截至6月」、预测恢复
   - 「已产生费用」→「**26年费用**」；「去年费用」紫色折线 →「**25年费用**」半透明紫条形；预测费用保持虚线框；x 轴固定 1-12 月避免重复
   - 「总费用构成」单饼图 → **两个环形图**（bdPie26=结算金额按需求类型 / bdPie25=去年支出费用按需求类型；value 存原始金额，修原用 totalBudget 反推金额的 bug）
3. **质量抽审**：月份筛选「**全部**」时详情区按组聚合全月**合计**（计数求和；初审通过率=Σ初审通过/Σ审核；高质量报价=Σ(率×报价量)/Σ报价量，仅含该字段月份），不逐月展开；选具体月才展开该月。新增 `fmtQr` 按字段名格式化（修旧 `v>1` 启发式把 `问题采集员=1` 显示成 100%）；「分析」Tab 的 tc4a/tc4b 趋势图保留按月（趋势图性质）
- 验证：`node --check` 通过；`_smoke_test.js` 5/5 通过；增强验证 26 项通过（截至6月/预测恢复/双环形图/合计口径/单月展开）

### 数据现状提醒（2026-08-17）
- ✅ **闭环状态(col29) 已填**（1-7月任务='是'、8月未填）→ 任务清单"闭环状态"列取模板真实值（缺省回退完成率推断）
- ✅ **采集完成时间(col22) 2条**（7月江苏比价=2026-07-31、浙江比价=2026-08-16）→ 任务清单新增"采集完成"列
- ✅ **约定采集完成时间(dueMonth) 已填**（MMDD 数值：730=7月30→07月、810=8月10→08月、830=8月30→08月、930=9月30→09月）→ 当月采集完成率 / "约定完成"列有值
- ✅ **结算含 7月**（新增4条：市场价-价格采集 282775 / 价格更新 834.9 / 全系列报价单 2850 / 信息价采集 26277.5）+ 6月信息价采集补填 → **结算 sum=1,218,356.6**（08-10 为 902,077.2）；去年支出 sum=491.3万 不变
- ✅ **采集产能 Sheet 已填**（1-7月实际家数+员工人数；6-7月含计划/实际需求量+条数；8月仅计划量 51000/50000）
- ⏳ **加工预审 col9 价格审核结果 仍全空** → 加工审核"价格审核通过率"显示"—"，待用户填数
- ⏳ **7/8月采集数据未填**（7月两任务仅填采集完成时间无完成量、8月三任务仅基础列）→ 前端对应显示"—"，属未完成态
...
```
待提交：08-18 流程卡按月联动（结算与成本 截至选中月累计 / 加工审核发布 月度入库率·新老供应商·审核总量）+ 加工预审 by_month 月度分解
```
- 历史已推送：`df86774`（对话登记北京/湖北 + 任务清单改版 + 台账补充）、`7118337`、`df5f5dc`（08-17 模板适配）、`d3fb1bf`（前端三模块可视化优化）、`55e0df3`、`89a3740`、`73c44e6`、`e25cbf4`、`4dcbbbb`、`b9cf068`、`05d89e3`
- 本次改动：parse_template.py（`by_month` 扩展 new_enterprise/old_enterprise/price_audit/newRate/oldRate）、index.html（renderFlowKPIs 加工审核按月 + 结算按月累计 + 审核总量按月）、data.json（新解析器重生成，dataEmbed 同步）
- **问题闭环卡：issueFeedback 无月份维度**（模板问题反馈表为全局标签-值对），暂保持全局，待用户补充月度问题数据或确认口径
- 验证：月度联动 node 测试 10/10 通过（6月 vs 全部 数值联动）；冒烟 5/5
...
### 各项需求跟踪（29列，数据行从 row3 起，max_row=28；08-17 新增 采集完成时间 col22 / 闭环状态 col29，采集类列整体右移1位）
| col | 表头 | | col | 表头 |
|---|---|---|---|---|
| 2 | 月份 | | 16 | (无表头, 公式=O*3) |
| 3 | 需求项 | | 17 | 需求反馈率(=O/I) |
...
| 5 | 需求名称 | | 19 | 需求下发团队时间 |
| 6 | 需求来源 | | 20 | 差距原因 |
| 7 | **约定采集完成时间**(dueMonth) | | 21 | 4月承诺采集完成率 |
| 8 | 收到数量(含多家) | | 22 | **采集完成时间**(collectedTime,新，日期序列号→YYYY-MM-DD) |
| 9 | **材料数量**(matQty) | | 23 | **实际采集完成量**(collected) |
| 10 | 产品承诺需求发出时间 | | 24 | 实际采集材料 |
| 11 | 实际需求收到时间(日期序列) | | 25 | **采集完成率**(=W/M) |
| 12 | **需求发布及时率**(timelyRate) | | 26 | **无效数据**(invalidData) |
| 13 | **下发数量含多家**(totalPoints) | | 27 | **无效数据占比**(=Z/N) |
| 14 | **下发材料**(itemCount) | | 28 | **发布数据**(published) |
| 15 | 预审问题数据 | | 29 | **闭环状态**(closed,新，值'是'/'否') |

公式列：col13下发数量`=N*3`、col14下发材料`=I-O`、col23采集完成量`=X*3`(山东)/`=I-Z`(广西)、col25完成率`=W/M`、col18准确率`=N/I`、col27无效率`=Z/N`、col8收到数量`=I*3`
